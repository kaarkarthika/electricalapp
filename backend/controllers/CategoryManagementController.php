            ...
            if ($_FILES['CategoryManagement']['error']['category_image'] == 0) {
                 //echo"sds";die;               
                  ...
                  //  $model->scenario = 'imageUploaded';
                    ...
                    $image_name = 'images/electrical/' . $model->file->basename . $rand . "." . $model->file->extension;
                    $model->file->saveAs($image_name);
                    $model->category_image = $image_name;
            }
            if ($_FILES['CategoryManagement']['error']['category_icon'] == 0) {
                    $rand = rand(0, 99999);
                    $model->file = UploadedFile::getInstance($model, 'category_icon');
                    $image_name = 'images/electrical/' . $model->file->basename . $rand . "." . $model->file->extension;
                    $model->file->saveAs($image_name);
                    $model->category_icon = $image_name;
            }
            if ($model->save()) {
            ...
            if ($_FILES['CategoryManagement']['error']['category_image'] == 0) {
             ...
             //print_r($category_image);die;
              ...
              if ($_FILES['CategoryManagement']['error']['category_icon'] == 0) {
                    $rand = rand(0, 99999);
                    $model->file = UploadedFile::getInstance($model, 'category_icon');
                    $image_name1 = 'images/electrical/' . $model->file->basename . $rand . "." . $model->file->extension;
             ...
             $command = Yii::$app->db->createCommand("UPDATE category_management SET category_name='$category_name', category_desc='$category_desc', slug='$slug', active_status='$active_status',category_image='$category_image',category_icon='$category_image1' WHERE auto_id=".$Categoryid);
              $command->execute();
              }
              else if($category_image!=''){
                $command = Yii::$app->db->createCommand("UPDATE category_management SET category_name='$category_name', category_desc='$category_desc', slug='$slug', active_status='$active_status',category_image='$category_image' WHERE auto_id=".$Categoryid);
                $command->execute();

              }
              else if($category_image1!=""){
               
                 $command = Yii::$app->db->createCommand("UPDATE category_management SET category_name='$category_name', category_desc='$category_desc', slug='$slug', active_status='$active_status',category_icon='$category_image1' WHERE auto_id=".$Categoryid);
                 $command->execute();
              }
            else
            {
            $command = Yii::$app->db->createCommand("UPDATE category_management SET category_name='$category_name', category_desc='$category_desc', slug='$slug', active_status='$active_status' WHERE auto_id=".$Categoryid);
              $command->execute();
              }
