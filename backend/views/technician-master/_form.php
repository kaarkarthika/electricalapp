            ...
            <div class="panel-body   ">
               ...
               <div class="row">
                  <div class='col-sm-6 form-group' >
                     <?= $form->field($model, 'technician_name')->textInput(['maxlength' => true,'placeholder'=>'Name','class'=>'form-control upper'])?>
                  </div>
                  <div class='col-sm-6 form-group' >
                     <?= $form->field($model, 'address')->textarea(['rows' => 6,'class'=>'form-control upper','placeholder'=>'Address'])?>
                  </div>
                  <div class='col-sm-6 form-group' >
                   <?= $form->field($model, 'phone_no')->textInput(['class'=>'form-control', 'id'=>'phone_no','maxlength' => 10,'onkeypress'=>'return isNumberKey(event)'])?>
                  </div>
                   <div class='col-sm-6 form-group' >
                      <?= $form->field($model, 'email_id')->textInput(['maxlength' => true])?>
                  </div>
                  <div class='col-sm-6 form-group'>
                     <?= $form->field($model, 'technician_image')->fileInput(['id'=> 'technician_image','maxlength' => true,'class'=>'form-control upper'])?>
                  <?php
                  if($model->technician_image!=""){
                   $base=Url::base(true);
                   $video_image=$base."/".$model->technician_image;
                   ?>
                   <img src="<?php echo $video_image;?>" style="width:70px;height:70px;">
                   <?php
                   }else{
                     echo "No Images !!!";
                   }?>
