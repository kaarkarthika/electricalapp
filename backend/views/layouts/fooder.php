...
?>
      <footer class="main-footer">
        ...
        <strong>&copy; <?= date('Y') ?>  Pooja Electrical </strong> All rights reserved.
      </footer>
...
<script>
    ...
    $(document).on('click', '.modalDelete2', function(e){
        e.preventDefault();
         var url = $(this).val();
            ...
            $('#textContent').html('<h4>Are you sure you want to <span style="color:red;">Approve</span> this Type ?</h4>');
            $('#modal').modal('show').find('#modalContent').load();
    });
 $(document).on('click', '.modalDelete3', function(e){
        e.preventDefault();
         var url = $(this).val();
            $('.deletatag').attr("href", url);
            $('#customheader').html('<span style="color:red"> <i class="fa fa-share" aria-hidden="true"></i> Start</span>');
            $('#textContent').html('<h4>Are you sure you want to <span style="color:red;">Cancel</span> this Type ?</h4>');
            $('#modal').modal('show').find('#modalContent').load();
    });
